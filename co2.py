...
try:
    ...
    while True:
        X = mh_z19.read_all()
        CO2_VALUE = X["co2"]
        TEMP_VALUE = X["temperature"]
        t = time.localtime()
        print(CO2_VALUE, "at", time.strftime("%Y-%m-%d %H:%M:%S", t))
        with canvas(DEVICE) as draw:
            draw.text((0, 0), "CO₂", font=FONT, fill="white")
            draw.text((70, 0), str(CO2_VALUE), font=FONT, fill="white")
            draw.text((0, 30), "Tmp", font=FONT, fill="white")
            draw.text((70, 30), str(TEMP_VALUE), font=FONT, fill="white")
            draw.text((100, 30), "°C", font=FONT, fill="white")

        time.sleep(3)
        if CO2_VALUE > CO2_CRITICAL:
            set_red()
        elif CO2_VALUE > CO2_WARNING:
            set_yellow()
        elif CO2_VALUE < CO2_LOW:
            set_green()
            time.sleep(1)
            set_none()
        else:
